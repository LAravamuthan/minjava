...
public class IntervalTreeFile extends IndexFile implements GlobalConst {
 ...
 private void  _destroyFile(PageId pageno) 
  ...
  public void insert(KeyClass key, RID rid) 
	...
	newRootPage=new IntervalTLeafPage( headerPage.get_keyType());
  ...
  private KeyDataEntry  _insert(KeyClass key, RID rid,  
	...
	{
	  IntervalTLeafPage currentLeafPage = 
	    new IntervalTLeafPage(page, headerPage.get_keyType() );
  ...
  IntervalTLeafPage findRunStart (KeyClass lo_key, 
			   RID startrid)
    throws IOException, 
	   IteratorException,  
	   KeyNotMatchException,
	...
	while((tmpEntry!=null) && (IntervalT.keyCompare(key,tmpEntry.key)>=0)) { 
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, rid)) ) {
		...
		return null;
              ...
              siblingPage=new IntervalTLeafPage(pinPage(siblingPageId), 
					 headerPage.get_keyType());
		...
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  IntervalTLeafPage nextLeafPage=new IntervalTLeafPage(
