...
public class BlockNestedLoopsJoins extends Iterator {
  ...
  private int inner_buf_pgs;        // # of inner buffer pages available.
  private int outer_buf_pgs;        // # of outer buffer pages available.
  ...
  private Heapfile hf;
  private Scan inner;
  private Queue<Tuple> outBuffer;  // outer buffer for NLJ
  private Queue<Tuple> inBuffer;   // inner buffer for NLJ
  private int outPos;        // outer buffer position
  private int inPos;         // inner buffer position
   ...
   * @throws LowMemException     exception from small buffer size
   */
  public BlockNestedLoopsJoins(AttrType in1[],
                               int len_in1,
                               short t1_str_sizes[],
                               AttrType in2[],
                               int len_in2,
                               ...
                               FldSpec proj_list[],
                               int n_out_flds
  ) throws IOException, NestedLoopException, LowMemException {
    ...
    if (n_buf_pgs < 2) {
      // Abort task if n_buf_pgs < 2
      throw new LowMemException("Not enough buffers to run Join: NestedLoopJoins.java");
    } else {
      // split:  outer: 80%; inner: 20%
      outer_buf_pgs = ((80 * n_buf_pgs) / 100);
      inner_buf_pgs = n_buf_pgs - outer_buf_pgs;
      inBuffer = new BufMgr(inner_buf_pgs, "Clock");
      outBuffer = new BufMgr(inner_buf_pgs, "Clock");
      inPos = 0;
      outPos = 0;
    }
