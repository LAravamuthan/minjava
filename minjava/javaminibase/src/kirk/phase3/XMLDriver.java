...
/*
 * This class contains parsing function which are responsible for parsing the
 * XML into tags
 * 
 */
...
class XMLLineParser {
    ...
    // This will extract all the tags from a XML line contained in < and >
    // characters
    public List<String> ExtraxtContent(String line) {
        ...
        if (firstspace < 0) {
            String justelem = line.length() > 5 ? line.substring(0, 5) : line; // trim if lenght is greater than 5
                                                                               // characters
    ...
    // This will parser an entire XML line and return a list of string which will
    // contain the tags
...
class XMLDriver implements GlobalConst {
    ...
    /**
     * XMLDriver costructor for XML file import
     * @param dbfileName - database file path/name
     * @param fileName - XML import file path/name
     */
    public XMLDriver(String dbfileName, String fileName) {
        try {
            ...
            this.stack = new Stack<XMLNode>(); // create a stack for pushing and popping the nodes of XML this will help
            // in assigning proper interval values
            this.IntervalNo = 1;
            this.rid = null;
            this.hpfilename = "XMLtags.in";

            // Attrtypes , these are used for the sethdr function which helps set the offset
            ...
            String dbpath = dbfileName;

            SystemDefs sysdef = new SystemDefs(dbpath, MINIBASE_DB_SIZE, MINIBASE_BUFFER_POOL_SIZE, "Clock");
            ...
            try {
                this.xmlDriverTuple.setHdr((short) 3, Stypes, Ssizes); // set the header for the tuple to be stored in
                // heap file
            } catch (Exception e) {
                System.err.println("*** error in Tuple.setHdr() ***");
                e.printStackTrace();
    ...
    /**
     * XMLDriver costructor for existing DB file
     * @param dbfileName - database file path/name
     */
    public XMLDriver(String dbfileName) {
        // in assigning proper interval values
        this.IntervalNo = 1;
        this.rid = null;
        this.hpfilename = "XMLtags.in";
System.out.println(("KIRK: ---->  IN XMLDriver with dbfileName:" + dbfileName));
        // Attrtypes , these are used for the sethdr function which helps set the offset
        // to where the data is stored
        this.Stypes = new AttrType[3];
        this.Stypes[0] = new AttrType(AttrType.attrInteger);
        ...
        try {
            this.hpfile = new Heapfile(this.hpfilename);
          //  this.rid = new RID(new PageId(0),0);
            System.out.println("KIRK: Heapfile has " + this.hpfile.getRecCnt() + " records.");
            //this.rid = new RID()KIRK
        } catch (Exception e) {
            System.err.println("*** error in Heapfile constructor ***");
            e.printStackTrace();
        }
        ...
        this.SizeofTuple = this.xmlDriverTuple.size();
        try {
            this.xmlDriverTuple = this.hpfile.getRecord(this.rid);
        } catch (Exception e) {
            System.err.println("*** error in Tuple.getRecord() ***");
            e.printStackTrace();
        }
    ...
    private void PushOnStack(String nodename) {
        ...
        if (this.stack.empty()) {
            ...
            xmlNode = new XMLNode(this.stack.peek().getStartOfInterval(), this.IntervalNo, nodename); // otherwise get
                                                                                                      // everyone parent
                                                                                                      // and store it
        }
    ...
    // pop a node from the stack, when you pop a node you get the end interval of
    // the tag and you can now write it to the heap file
    ...
    // Write all the tags of an XML line into the heap file (either push it or pop
    // it from the stack)
    ...
    // When a node is popped from the stack it has to be pushed in the heap file
    // (now we can actually store it in a heap file becuase we have all the
    ...
    private void PushNodeToHeapFile(XMLNode xmlNode) {
        ...
        try {
            this.rid = this.hpfile.insertRecord(this.xmlDriverTuple.returnTupleByteArray()); // store it in the heap
                                                                                             // file
        } catch (Exception e) {
            System.err.println("*** error in Heapfile.insertRecord() ***");
            e.printStackTrace();
    ...
    // This function reads each line of an XML and passes the data to the parsing
    // function and at the end it stores it in the heap file
    public NodeContext ReadFileLbyLStoreInHeapFile() {
        ...
        try {
            ...
            this.reader.close();
            tgpr = new NodeContext(this.hpfile, this.rid, this.hpfilename, GetAttrType(), GetStrSizes(),
                    GetTupleSize());
    ...
    // This function takes an existing database file and connnects it to
    // the heap file
    public NodeContext OpenExistingDBinHeapFile() {
        NodeContext tgpr = null;
    ...
    // This function generates the FLdSpec array which give the projections. i.e
    // after joining two table which colums to keep from the outer and inner table.
    // we keep all the column from the outer table and only omit those columns which
    // are passed in the ltfieldtoomit list.
    // This generate the condexpr required to see the containment or equality
    // criteria
    ...
    // This funtion joins the attr type of two table depending upon the projection
    // specified
    ...
    // This function joins the attrstrsizes of the two tables depending upon the
    // field to omit list
    ...
    public NodeContext JoinTwoFields(NodeContext tag1, int joinfieldno1, NodeContext tag2, int joinfieldno2,
            List<Integer> ltfieldtoomit, boolean parentchildflag, boolean ContainOrEquality) {
        ...
        outFilter = GenerateCondExpr(joinfieldno1, joinfieldno2, ContainOrEquality); // generate condexpr for the two
                                                                                     // joining fields
        ...
        /*
         * System.out.println("jere"); System.out.printf("%s %s\n", joinfieldno1,
         * joinfieldno2); for(int i=0;i<JoinedTagsize.length;i++) {
        ...
        FileScan fscan = null;
        try {
            fscan = new FileScan(taghpfilename1, tagattrtype1, tagattrsize1, (short) outer, outer, projlist_tag1, null); // file
                                                                                                                         // scan
                                                                                                                         // pointer
        } catch (Exception e) {
        ...
        try {
            ...
            nlj = new NestedLoopsJoins(tagattrtype1, outer, tagattrsize1, tagattrtype2, inner, tagattrsize2, 10, fscan,
                    taghpfilename2, outFilter, null, projlist_tag2, outer + inner - (3 * ltfieldtoomit.size()));
        } catch (Exception e) {
            ...
            while ((temptup = nlj.get_next()) != null) {
                if (parentchildflag) {
                    if (temptup.getIntervalFld(2).get_s() == temptup.getIntFld(4)) {
                        JoinedTagRID = JoinedTaghpfile.insertRecord(temptup.returnTupleByteArray()); // insert the
                                                                                                     // joined record
                                                                                                     // into a new heap
                                                                                                     // file
                    }
                } else {
                    JoinedTagRID = JoinedTaghpfile.insertRecord(temptup.returnTupleByteArray()); // get the tag RID of
                                                                                                 // the resulting tuple
                }
            }
        } catch (Exception e) {
        ...
        return new NodeContext(JoinedTaghpfile, JoinedTagRID, JoinedTaghpfilename, JoinedTagAttrtype, JoinedTagsize,
                JoinedTagTupSize);
    }

    // Extract all the data from the heap file which match to the given tags in
    // tagnames array and create new heap file for each of them
    // basically this function creates new heap files where each individual heap
    // file contains only a particular tag value
    ...
    public NodeContext[] ExtractTagToHeap(NodeContext tag_params, String[] tagnames) {
            ...
            try {
                ...
                for (int i = 0; i < tot_len; i++) {
                    if (tag.equals(tagnames[i])) {
                        try {
                            ridtostore[i] = heaptostore[i].insertRecord(tup.returnTupleByteArray()); // inset the data
                                                                                                     // into a separate
                                                                                                     // heap file
                            break;
                        } catch (Exception e) {
        ...
        for (int i = 0; i < tot_len; i++) {
            tag_pars[i] = new NodeContext(heaptostore[i], ridtostore[i], hpfilenm[i], Stps, Sszs, TupSize); // generate
                                                                                                            // tag pars
                                                                                                            // data type
                                                                                                            // for each
                                                                                                            // of the
    ...
    // This gives which fields can be joined for resultant join and a new joined
    // table
    ...
    // prepare the list of fields which are reuqired to be ommited from the second
    // table , as in the common fields
    ...
    public TagparamField Query(NodeContext[] ExtractTags, String[] Joins) {
        ...
        for (int i = 0; i < Joins.length; i++) {
            ...
            TempTagPar = JoinTwoFields(ExtractTags[ftfld], 2, ExtractTags[scfld], 2, ltfieldtoomit, relflag, true); // create
                                                                                                                    // a
                                                                                                                    // join
                                                                                                                    // of
                                                                                                                    // two
            ...
            if (ResultTagPar != null) {
                ...
                ltfieldtoomit = GetMyOmitList(FieldTracker, ltfieldtoomit, ftfld, scfld); // get which fields need to be
                                                                                          // omitted
                ResultTagPar = JoinTwoFields(ResultTagPar, fields[0], TempTagPar, fields[1], ltfieldtoomit, false,
                        false); // join the tags to a resultant table
    ...
    // This function joins the two tables obtained from two queires and finally
    // gives you result which we print as query result obtained
    ...
    // check which queries are possible and give out two such number which can be
    // used to split the joins
    // so for each query we have a certain number of ancesteer descendant or parent
    // child joins given we try to create split in the joins
    // and try to see whether the split creates will be able to form a successfull
    // join on its own, when we get two such successful split values we
    // return the two number and those two number will be used to get our two query
    // plans
    public int[] querypossible(String[] NumberofJoins) {
        ...
        while (splitter < lenoflist) {
            ...
            for (int i = splitter; i < lenoflist; i++) {
                tempstr = NumberofJoins[i].split(" ");
                if (seclot.size() != 0) {
                    if (seclot.indexOf(Integer.parseInt(tempstr[0])) == -1
                            && seclot.indexOf(Integer.parseInt(tempstr[1])) == -1) {
                        possiblesec = false;
                        break;
                    }
                }
    ...
    public NodeContext[] ReadQueryAndExecute(NodeContext MainTagpair, String Queryfilename) {
        ...
        for (int i = 1; i <= numberofnodes; i++) {
            searchtags[i - 1] = querylinelist.get(i).length() > 5 ? querylinelist.get(i).substring(0, 5)
                    : querylinelist.get(i);
        }
        ...
        /*
         * try { MainTagpair.GetHeapFile().deleteFile(); } catch(Exception e) {
         * e.printStackTrace(); }
         */
    ...
    // perform a select using one pattern tree
    public NodeContext[] SelectQuery(NodeContext MainTagpair, PatternTree ptree) {
        NodeContext QueryResult = null;
        NodeContext[] finalresult = null;

        System.out.println("KIRK: Entering SelectQuery");

        int numberofnodes = ptree.getNumNodes();
        ArrayList<String> searchtagList = ptree.getTags();
        String[] searchtags = new String[numberofnodes];
        for(int i=0; i<numberofnodes; i++) {
            searchtags[i] = searchtagList.get(i);
            System.out.println("KIRK: tags:  "+searchtags[i]);
        }

        finalresult = ExtractTagToHeap(MainTagpair, searchtags); // get all the heap file for each tag

        System.out.println("KIRK: Select complete");
        return finalresult;
    }
