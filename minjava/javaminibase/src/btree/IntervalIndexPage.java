 ...
 */
public class IntervalIndexPage extends IntervalSortedPage{
     ...
     */
    public IntervalIndexPage(PageId pageno, int keyType)
            throws IOException,
     ...
     *@exception ConstructPageException error when BTIndexpage constructor
     */
    public IntervalIndexPage(Page page, int keyType)
            throws IOException,
            ConstructPageException
    {
        super(page, keyType);
     ...
     *@exception ConstructPageException error when BTIndexpage constructor
     */
    public IntervalIndexPage( int keyType)
            throws IOException,
            ConstructPageException
    {
        super(keyType);
            ...
            throws IndexFullDeleteException
    {
        ...
        try {
            ...
            if ( intervalT.keyCompare(key, entry.key)<0)// && intervalT.keyCompare(key,entry.key).getE()<0)
                //it is supposed to not smaller than first key
                throw new IndexFullDeleteException(null, "First key is bigger");


            while (intervalT.keyCompare(key, entry.key)>0)// && intervalT.keyCompare(key,entry.key).getE()>0) {
            {
                entry = getNext(rid );
                if (entry == null)
                    break;
     ...
     */
    boolean redistribute(IntervalIndexPage indexPage, IntervalIndexPage parentIndexPage,
