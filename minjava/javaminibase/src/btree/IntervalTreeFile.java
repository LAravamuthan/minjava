...
public class IntervalTreeFile extends IndexFile
     ...
     * @return  Return a intervalTreeHeaderPage object that is the header page
     *          of this btree file.
     */
...
//            headerPage.set_keyType((IntervalType)key);
            ...
            FreePageException
    {
        ...
        IntervalSortedPage sortedPage;
        Page page=pinPage(pageno) ;
        sortedPage= new IntervalSortedPage( page, headerPage.get_keyType());

        if (sortedPage.getType() == NodeType.INDEX) {
            IntervalIndexPage indexPage= new IntervalIndexPage( page, headerPage.get_keyType());
            RID      rid=new RID();
            PageId       childId;
            KeyDataEntry entry;
...

    {
        ...
        if (headerPage.get_rootId().pid == INVALID_PAGE) {
            ...
            IntervalLeafPage newRootPage;
            RID dummyrid;

            newRootPage=new IntervalLeafPage( headerPage.get_keyType());
        ...
        if (newRootEntry != null)
        {
            ...
            newRootPage = new IntervalIndexPage(headerPage.get_keyType());
            newRootPageId=newRootPage.getCurPage();
...

    {
        ...
        IntervalSortedPage currentPage;
        Page page;
        KeyDataEntry upEntry;


        page=pinPage(currentPageId);
        currentPage=new IntervalSortedPage(page, headerPage.get_keyType());
        ...
        if(currentPage.getType() == NodeType.INDEX) {
            IntervalIndexPage  currentIndexPage=new IntervalIndexPage(page,
            ...
            currentIndexPage= new  IntervalIndexPage(pinPage(currentPageId),
                    headerPage.get_keyType() );
        ...
        {
            IntervalLeafPage currentLeafPage =
                    new IntervalLeafPage(page, headerPage.get_keyType() );
            ...
            {
                IntervalLeafPage rightPage;
                rightPage=new IntervalLeafPage(rightPageId, headerPage.get_keyType());
     ...
     *@return return a IntervalLeafPage instance which is pinned.
     *        null if no key was found.
     */

    IntervalLeafPage findRunStart (KeyClass lo_key,
                             RID startrid)
            throws IOException,
            IteratorException,
            KeyNotMatchException,
            ...
            UnpinPageException
    {
        IntervalLeafPage  pageLeaf;
        IntervalIndexPage pageIndex;
        Page page;
        IntervalSortedPage  sortPage;
        PageId pageno;
        ...
        page= pinPage(pageno);
        sortPage=new IntervalSortedPage(page, headerPage.get_keyType());
        ...
        while (sortPage.getType() == NodeType.INDEX) {
            pageIndex=new IntervalIndexPage(page, headerPage.get_keyType());
        ...
        pageLeaf = new IntervalLeafPage(page, headerPage.get_keyType() );
        ...
        while (curEntry==null) {
            ...
            pageno = nextpageno;
            pageLeaf=  new IntervalLeafPage( pinPage(pageno), headerPage.get_keyType());
            curEntry=pageLeaf.getFirst(startrid);
        }
            ...
            IteratorException
    {
        ...
        while ( true ) {
            ...
            while ( entry == null) { // have to go right
                ...
                leafPage=new IntervalLeafPage(pinPage(nextpage),
                        headerPage.get_keyType() );
            ...
            leafPage=new IntervalLeafPage(pinPage(nextpage), headerPage.get_keyType());
            ...
            IOException
    {
        ...
        if (sortPage.getType()==NodeType.LEAF ) {
            ...
            PageId nextpage;
            IntervalLeafPage leafPage;
            leafPage=new IntervalLeafPage(page, headerPage.get_keyType());
            ...
            {
                // WriteUpdateLog is done in the IntervalLeafPage level - to log the
                // deletion of the rid.
                ...
                if ( leafPage.delEntry(new KeyDataEntry(key, rid)) ) {
                    ...
                    if ( (4+leafPage.available_space()) <=
                        ...
                        // get a sibling
                        IntervalIndexPage  parentPage;
                        parentPage =
                                new IntervalIndexPage(pinPage(parentPageId),
                                        headerPage.get_keyType());

                        PageId siblingPageId=new PageId();
                        IntervalLeafPage siblingPage;
                        int direction;
                        ...
                        siblingPage=new IntervalLeafPage(pinPage(siblingPageId),
                                headerPage.get_keyType());
                        ...
                        if (siblingPage.redistribute( leafPage, parentPage,
                            ...
                            IntervalLeafPage leftChild, rightChild;
                            if (direction==-1) {
                            ...
                            if ( rightChild.getNextPage().pid != INVALID_PAGE) {
                                IntervalLeafPage nextLeafPage=new IntervalLeafPage(
        ...
        if (  sortPage.getType() == NodeType.INDEX ) {
            ...
            indexPage=new IntervalIndexPage(pinPage(currentPageId), headerPage.get_keyType());
            ...
            if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
                ...
                if (indexPage.numberOfRecords() == 0) {
                    IntervalSortedPage childPage;
                    childPage=new IntervalSortedPage(indexPage.getPrevPage(),
                            headerPage.get_keyType());
            ...
            if ((4 /*sizeof slot*/ +indexPage.available_space()) <=
                ...
                // get a sibling
                IntervalIndexPage  parentPage;
                parentPage=new IntervalIndexPage(pinPage(parentPageId),
                        headerPage.get_keyType());

                PageId siblingPageId=new PageId();
                IntervalIndexPage siblingPage;
                int direction;
                ...
                siblingPage=new IntervalIndexPage( pinPage(siblingPageId),
                        headerPage.get_keyType());
                ...
                if (siblingPage.redistribute(indexPage,parentPage,
                    ...
                    // merge the two children
                    IntervalIndexPage leftChild, rightChild;
                    if (direction==-1) {
                        leftChild = siblingPage;
...
public IntervalTFileScan new_scan(KeyClass lo_key, int condition)
        throws IOException,
        KeyNotMatchException,
        IteratorException,
        ConstructPageException,
...

{
    ...
    if (lo_key instanceof IntervalKey)
    {

        if (condition == 0) //before or contained by
        {
            hi_key = lo_key;
            lo_key = null;
        } else if (condition == 1) //equal to
        {
            hi_key = lo_key;
        }
    }
        ...
        UnpinPageException
    {
        ...
        if( trace!=null ) {
            ...
            KeyClass key;
            KeyDataEntry entry;
            sortedPage=new IntervalSortedPage( pinPage( id), headerPage.get_keyType());


            // Now print all the child nodes of the page.
            if( sortedPage.getType()==NodeType.INDEX) {
                IntervalIndexPage indexPage=new IntervalIndexPage(sortedPage,headerPage.get_keyType());
                ...
                IntervalLeafPage leafPage=new IntervalLeafPage(sortedPage,headerPage.get_keyType());
