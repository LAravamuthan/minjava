...
//        }
        if ( (key1 instanceof IntervalKey) && (key2 instanceof IntervalKey) )
        {
            ...
            returnkey.setS(((IntervalKey)key1).getKey().getS()
                    - (((IntervalKey)key2).getKey()).getS());
            returnkey.setE(((IntervalKey)key1).getKey().getS()
                    - (((IntervalKey)key2).getKey()).getS());

            if (returnkey.getS() >0 )             //the start of first key is greater than start of second key, or starts are equal but end is greater
                return 1;
            if (returnkey.getS() < 0 )
                return -1;
            if (returnkey.getS()==0 )
