...
//        {
//
//            return  (((IntegerKey)key1).getKey())
//                    - (((IntegerKey)key2).getKey());
//        }
        if ( (key1 instanceof IntervalKey) && (key2 instanceof IntervalKey) )
        {
            intervaltype returnkey = new intervaltype();
            returnkey.set_s(((IntervalKey)key1).getKey().get_s()
                    - (((IntervalKey)key2).getKey()).get_s());
            returnkey.set_e(((IntervalKey)key1).getKey().get_s()
                    - (((IntervalKey)key2).getKey()).get_s());

            if (returnkey.get_s() >0 )             //the start of first key is greater than start of second key, or starts are equal but end is greater
                return 1;
            if (returnkey.get_s() < 0 )
                return -1;
            if (returnkey.get_s()==0 )
                return  0;
            else
...
//
            ...
            ConvertException
    {
        ...
        try{
            ...
            {
//                Convert.setIntValue( ((IntervalKey) entry.key).getKey().getS(),0,data);
    ...
    {
        IntervalSortedPage sortedPage=new IntervalSortedPage(pageno, keyType);
        int i;
        i=0;
        if ( sortedPage.getType()==NodeType.INDEX ) {
            IntervalIndexPage indexPage=new IntervalIndexPage((Page)sortedPage, keyType);
            System.out.println("");
            ...
            ReplacerException
    {
        ...
        IntervalSortedPage sortedPage=new IntervalSortedPage(currentPageId, keyType);
        prefix=prefix+"       ";
        i++;
        if( sortedPage.getType()==NodeType.INDEX) {
            IntervalIndexPage indexPage=new IntervalIndexPage((Page)sortedPage, keyType);
