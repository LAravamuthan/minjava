        ...
        implements  GlobalConst
{
    ...
    //    System.out.println("Entered get_next()");
        KeyDataEntry entry;
        PageId nextpage;
        try {
            if (leafPage == null) {
            ...
            if (endkey != null) {
//                    int endkeystart = ((IntervalKey)endkey).getKey().getS();
//                    int endkeyend = ((IntervalKey)endkey).getKey().getE();
//                    int entrykeystart = ((IntervalKey)entry.key).getKey().getS();
//                    int entrykeyend = ((IntervalKey)entry.key).getKey().getS();
//                    System.out.println("endkeystart = " + endkeystart + " End key end = " + endkeyend + " entry key start = " + entrykeystart + " entry key end = " + entrykeyend);
                if (intervalT.keyCompare(entry.key, endkey) > 0) {
                    ...
                    leafPage = null;
         //           System.out.println("Now we are beyond end of scan");
                    return null;
                }
            }
  //          System.out.println("We are within scan range : returning the entry key ");
            return entry;
        }
        catch ( Exception e) {
            e.printStackTrace();
     ...
     */
    public  void IntervalTreeFileScan()
