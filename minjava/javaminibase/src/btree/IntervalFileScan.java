        ...
        implements  GlobalConst
{
    ...
    IntervalLeafPage leafPage;  //  leaf page containing current record
    ...
    KeyClass endkey;    // if NULL, then go all the way right else, stop when current record > this value (that is, implement an inclusive rang scan -- the only way to do a search for
                        //a single value).
    int keyType;
    int maxKeysize;
            ...
            throws ScanIteratorException
    {
        ...
        try {
            ...
            while ( entry == null ) {
                ...
                leafPage=new IntervalLeafPage(nextpage, keyType);

                entry=leafPage.getFirst(curRid);
            }

            /*
                Goal : To find all intervals which contain a given interval.
                2 cases are possible:
                a. The start of the given interval is less than the start of our target, but end is greater : s1 > s2 but e1 < e2
                b. The start of the given interval is less than our target.
            *?
             */
            if (endkey != null)            //when we scan, we look at all intervals such that their start is less than start of given node, as only they can be parents.
