...
public class ITIndexScan extends Iterator {
  ...
  throw new IndexException(e, "IndexScan.java: BTreeFile exceptions caught from BTreeFile constructor");
      ...
      try {
    indScan = (IntervalTFileScan) ITIndexUtils.IntervalTree_scan(selects, indFile);
      }
      catch (Exception e) {
  ...
  public ITIndexScan indexScan_copy()
  {
      ITIndexScan Insx = null;
      try
      {
            Insx =  new ITIndexScan(this._index, this._relName, this._indName, 
        this._types, this._s_sizes, this._noInFlds, this._noOutFlds, 
        this.perm_mat, this._selects, this._fldNum, this.index_only);
      }
      catch (Exception e)
      {
        e.printStackTrace();
  ...
  {
    if (!closeFlag) {
      if (indScan instanceof IntervalTFileScan) {
	try {
