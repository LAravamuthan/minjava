...
/** get the length of a tuple, call this method if you did
  ...
  */
  ...
  public IntervalType getIntervalField(int fldNo) throws IOException, FieldNumberOutOfBoundException
  {

    IntervalType inter;

    if ( (fldNo > 0) && (fldNo <= fldCnt)){
      inter = Convert.getIntervalFld(fldOffset[fldNo - 1], data);
      return inter;
    }
    else
     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
  ...
  public void setIntervalField(int fldNo, IntervalType val) throws IOException, FieldNumberOutOfBoundException
  {

    if((fldNo > 0) && (fldNo <= fldCnt)){

      if(val != null){
        Convert.setIntervalFld(val, fldNo, data);
      }
    }else{
       throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
    }
